...
public class Resilience4jController {
    ...
    @CircuitBreaker(name = RESILIENCE4J_INSTANCE_NAME, fallbackMethod = FALLBACK_METHOD)
    public CompletableFuture<Response<Boolean>> timeDelay(@PathVariable int delay) {
        return Mono.just(toOkResponse()).delayElement(Duration.ofSeconds(delay)).toFuture();
    }
